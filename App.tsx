...
// Design System / Theme based on Recipe 7 (Atmospheric)
const theme = {
  bg: 'bg-[#0a0502]',
  accent: 'text-[#ff4e00]',
  glass: 'bg-white/10 backdrop-blur-2xl border-white/20',
  glassDark: 'bg-black/30 backdrop-blur-3xl border-white/10',
  card: 'bg-[#151619] border-white/10',
  gradient: 'from-[#3a1510] via-[#0a0502] to-[#000]'
};
...
const ProgressBar = ({ progress }: { progress: number }) => (
  <div className="h-4 w-full bg-white/10 rounded-full overflow-hidden backdrop-blur-sm border border-white/5">
    <motion.div 
      ...
      className="h-full bg-gradient-to-r from-[#ff4e00] to-[#ff9e00] shadow-[0_0_20px_rgba(255,78,0,0.4)]"
    />
  </div>
);
...
const QuestButton = ({ 
  ...
  const variants = {
    primary: 'bg-[#ff4e00] hover:bg-[#ff6e00] text-white shadow-xl shadow-[#ff4e00]/20 font-black',
    secondary: 'bg-white/10 hover:bg-white/20 text-white backdrop-blur-md border border-white/20 font-bold',
    outline: 'border-2 border-[#ff4e00]/50 hover:border-[#ff4e00] text-white hover:bg-[#ff4e00]/10 font-bold',
  ...
  const Background = () => (
    <div className="fixed inset-0 pointer-events-none z-0">
      ...
      <motion.div 
        ...
        className="absolute top-[-20%] left-[-10%] w-[120%] h-[120%] bg-[radial-gradient(circle_at_center,rgba(255,78,0,0.1),transparent_70%)] blur-3xl" 
      />
      ...
      {[...Array(5)].map((_, i) => (
        <motion.div
          ...
          className="absolute bg-white/10 rounded-full blur-xl pill"
          style={{
      ...
      <div className={`min-h-screen ${theme.bg} text-white flex flex-col items-center justify-center p-6 font-sans relative overflow-hidden`}>
        ...
        >
          <div className="mb-8 flex justify-center gap-4">
            <div className="p-4 bg-[#ff4e00]/20 rounded-full border border-[#ff4e00]/40 backdrop-blur-xl animate-bounce">
              <Compass className="w-12 h-12 text-[#ff4e00]" />
            </div>
          </div>
          <h1 className="text-6xl md:text-8xl font-black mb-4 tracking-tighter text-transparent bg-clip-text bg-gradient-to-b from-white to-white/40">
            QUEST: SER vs ESTAR
          </h1>
          <p className="text-[#ff4e00] font-black uppercase tracking-[0.5em] mb-16 text-sm">30 Mysteries to solve</p>
          
          <div className="grid gap-12 sm:grid-cols-3 relative">
            {/* The Path Connector */}
            <div className="absolute top-1/2 left-0 right-0 h-1 bg-white/5 -z-10 hidden sm:block" />
            ...
            {LEVELS.map((level, idx) => (
              ...
              >
                <div className="w-24 h-24 rounded-full bg-gradient-to-tr from-[#ff4e00] to-[#ff9e00] flex items-center justify-center shadow-2xl shadow-[#ff4e00]/40 mb-6 group-hover:rotate-[360deg] transition-transform duration-700">
                  <Zap className="w-10 h-10 text-white" fill="white" />
                </div>
                
                <h3 className="text-2xl font-black mb-2">{level.title.split('.')[0]}</h3>
                <p className="text-white/60 text-xs font-bold uppercase tracking-widest leading-relaxed px-4">
                  {level.exercises.length} Exercises
                </p>

                {/* Card glow effect */}
                <div className="absolute inset-0 bg-[#ff4e00]/0 group-hover:bg-[#ff4e00]/5 rounded-[4rem] transition-colors -z-10" />
              </motion.button>
            ))}
          </div>
      ...
      <div className={`min-h-screen ${theme.bg} text-white flex flex-col items-center justify-center p-6 relative overflow-hidden`}>
        ...
        <motion.div initial={{ scale: 0.8, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} className={`text-center z-10 p-16 rounded-[4rem] max-w-xl w-full border border-orange-500/50 ${theme.glassDark}`}>
          <div className="relative mb-10">
            <Trophy className="w-40 h-40 text-[#ff4e00] mx-auto animate-bounce" />
            <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-64 h-64 bg-[#ff4e00]/20 rounded-full blur-[60px] animate-pulse -z-10" />
          </div>
          <h1 className="text-5xl font-black mb-4 tracking-tighter">LEVEL CONQUERED</h1>
          <p className="text-white/60 mb-12 text-xl font-bold">You are mastering the Spanish foundations, adventurer!</p>
    ...
    return (
      <div className={`min-h-screen ${theme.bg} flex flex-col items-center justify-center p-6 relative overflow-hidden`}>
        ...
        <motion.div 
          ...
          className={`relative z-10 p-12 rounded-[4rem] shadow-2xl border max-w-xl w-full text-center ${theme.glassDark}`}
        >
          <div className="w-24 h-24 bg-[#ff4e00] rounded-[2.5rem] flex items-center justify-center mx-auto mb-10 shadow-2xl shadow-[#ff4e00]/40 rotate-12">
            <HelpCircle className="w-14 h-14 text-white" />
          </div>
          <h2 className="text-4xl font-black mb-8 text-white tracking-tight">{currentLevel.title}</h2>
          <div className="p-8 bg-black/40 rounded-[2rem] border border-white/5 mb-12 text-left">
    ...
    <div className={`min-h-screen ${theme.bg} text-white font-sans flex flex-col relative overflow-hidden`}>
      ...
      <header className="p-4 md:p-8 flex items-center gap-10 max-w-5xl mx-auto w-full relative z-40">
        <motion.button 
          ...
          className="text-white/40 hover:text-[#ff4e00] transition-colors"
        >
          <X className="w-10 h-10" />
        </motion.button>
        <div className="flex-1">
          <div className="flex justify-between items-end mb-2">
            <span className="text-[10px] font-black tracking-widest text-[#ff4e00] uppercase">Quest Progression</span>
            <span className="text-[10px] font-mono text-white/40">{Math.round(progress)}% Complete</span>
          </div>
          <ProgressBar progress={progress} />
        </div>
        <div className={`flex items-center gap-3 text-[#ff4e00] font-black px-6 py-3 rounded-full border border-[#ff4e00]/40 ${theme.glass}`}>
        ...
        <AnimatePresence mode="wait">
          ...
          >
            ...
            <div className="flex items-center justify-center gap-4">
              <Compass className="w-6 h-6 text-[#ff4e00]" />
              <h2 className="text-xl md:text-2xl font-black uppercase tracking-[0.2em] text-center text-white">
            ...
            <motion.div 
              className={`flex items-center justify-center gap-8 p-16 rounded-[4rem] shadow-3xl relative group border-2 border-white/5 overflow-hidden ${theme.glassDark}`}
              style={{
                perspective: '1000px',
              ...
              <div className="relative z-10 flex flex-col md:flex-row items-center gap-10">
                 {currentExercise?.audioText && (
                  <motion.button 
                    ...
                    className="w-20 h-20 rounded-[2rem] bg-[#ff4e00] flex items-center justify-center shadow-2xl shadow-[#ff4e00]/40 hover:bg-[#ff6e00] transition-colors"
                  >
                    <Volume2 className="w-10 h-10 text-white" />
                  </motion.button>
                )}
                <div>
                   <p className="text-5xl md:text-7xl font-black tracking-tight text-white leading-tight">
                    {currentExercise?.prompt.split('___').map((part, i, arr) => (
                      <React.Fragment key={i}>
                        {part}
                        {i < arr.length - 1 && (
                          <span className="inline-block min-w-[2.5em] h-[1.2em] border-b-8 border-[#ff4e00] mx-4 relative top-2 bg-black/20 rounded-lg" />
                        )}
                      </React.Fragment>
                    ))}
                  </p>
                  <p className="text-white/40 mt-8 text-2xl font-medium font-serif italic">{currentExercise?.translation}</p>
                </div>
              </div>
            </motion.div>
                ...
                <button
                  ...
                  className={`
                    ...
                    ${selectedOption === choice 
                      ? 'border-[#ff4e00] bg-[#ff4e00]/20 text-[#ff4e00] shadow-2xl shadow-[#ff4e00]/20' 
                      : 'border-white/10 hover:border-white/30 bg-white/5 text-white/80'}
                    ${feedback && choice === currentExercise.target ? 'border-emerald-500 bg-emerald-500/20 text-emerald-400' : ''}
                    ${feedback === 'incorrect' && selectedOption === choice ? 'border-rose-500 bg-rose-500/20 text-rose-400' : ''}
                  `}
                >
                  {choice}
                  {selectedOption === choice && !feedback && (
                    <motion.div 
                      ...
                      className="absolute right-8 w-8 h-8 rounded-full bg-[#ff4e00] flex items-center justify-center text-white text-sm"
                    >
                      ✓
                    </motion.div>
                  )}
